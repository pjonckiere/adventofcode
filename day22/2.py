...
def turn(index, att_stack, attacks, hp, mana, b_hp, b_dam, mana_usage):
    ...
    if index not in att_stack:
            att_stack[index] = []
    for t in range(1, attacks[a][5] + 1):
        if index + t not in att_stack:
            att_stack[index + t] = []
        if a not in att_stack[index + t]:
...
stable = 0
while stable < 100000000:
    arr = fight(attacks_init, player_hp, player_mana, boss_hp_init, boss_damage_init)
    if arr[0] < lowest_mana:
        lowest_mana = arr[0]
        print arr[1]
        print lowest_mana
        stable = 0
